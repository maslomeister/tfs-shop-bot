 ...
 * @return {String} text
 */
function ticketMsg(name: string, message: { text: any; from: { first_name: any } }) {
...
function privateReply(ctx: Context, msg: any = {}) {
  ...
  // Msg to other end
  middleware.msg(ctx.session.modeData.userid, ticketMsg(` ${ctx.session.modeData.name}`, msg), {
    parse_mode: "html",
    reply_markup: {
      html: "",
      inline_keyboard: [
...
function chat(ctx: Context) {
  ...
  try {
    ...
    let userid = replyText.match(new RegExp("#T" + "(.*)" + " " + cache.config.language.from));
    ...
    db.getOpen(userid[1], ctx.session.groupCategory, function (ticket: { userid: string }) {
      if (userid === null || userid === undefined) {
        return;
      }
      ...
      if (ticket.userid.indexOf("WEB") > -1) {
        try {
          ...
          cache.io.to(socketId).emit("chat_staff", ticketMsg(name[1], ctx.message));
        } catch (e) {
          // To staff msg error
          middleware.msg(
            ctx.chat.id,
